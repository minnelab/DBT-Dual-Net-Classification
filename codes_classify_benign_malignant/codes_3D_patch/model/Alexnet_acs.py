...
class AlexNet3D(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.features = nn.Sequential(
            #ACSConv(1, 64, kernel_size=11, stride=4, padding=5),  # Adjust padding to keep feature size reasonable
            SoftACSConv(1, 64, kernel_size=11, stride=4, padding=5),
            nn.ReLU(inplace=True),
            nn.MaxPool3d(kernel_size=3, stride=2, padding=1),  # Adjust padding to keep feature size reasonable
            #ACSConv(64, 192, kernel_size=5, padding=2),
            SoftACSConv(64, 192, kernel_size=5, padding=2),
            nn.ReLU(inplace=True),
            nn.MaxPool3d(kernel_size=3, stride=2, padding=1),
            #ACSConv(192, 384, kernel_size=3, padding=1),
            SoftACSConv(192, 384, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            #ACSConv(384, 256, kernel_size=3, padding=1),
            SoftACSConv(384, 256, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            #ACSConv(256, 256, kernel_size=3, padding=1),
            SoftACSConv(256, 256, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.MaxPool3d(kernel_size=3, stride=2, padding=1),
        )

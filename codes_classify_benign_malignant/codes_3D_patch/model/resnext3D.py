...
class Bottleneck(nn.Module):
    """
    ResNeXt bottleneck type C for 3D images
    """
    expansion = 4
    ...
    def __init__(self, inplanes, planes, baseWidth, cardinality, stride=1, downsample=None):
        ...
        self.conv1 = SoftACSConv(inplanes, D*C, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn1 = nn.BatchNorm3d(D*C)
        #self.conv2 = nn.Conv3d(D*C, D*C, kernel_size=3, stride=stride, padding=1, groups=C, bias=False)
        self.conv2 = SoftACSConv(D*C, D*C, kernel_size=3, stride=stride, padding=1, groups=C, bias=False)
        self.bn2 = nn.BatchNorm3d(D*C)
        #self.conv3 = nn.Conv3d(D*C, planes * 4, kernel_size=1, stride=1, padding=0, bias=False)
        self.conv3 = SoftACSConv(D*C, planes * 4, kernel_size=1, stride=1, padding=0, bias=False)
...
class ResNeXt(nn.Module):
    """
    ResNeXt optimized for 3D images.
    """
    def __init__(self, baseWidth, cardinality, layers, num_classes):
        ...
        self.conv1 = SoftACSConv(1, 64, kernel_size=(7, 7, 7), stride=(2, 2, 2), padding=(3, 3, 3), bias=False)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                SoftACSConv(self.inplanes, planes * block.expansion,kernel_size=1,stride=stride,bias=False),
...
def resnext50(baseWidth, cardinality, num_classes):
    """
    Construct ResNeXt-50 for 3D images.
    """
...
def resnext101(baseWidth, cardinality, num_classes):
    """
    Construct ResNeXt-101 for 3D images.
    """
...
def resnext152(baseWidth, cardinality, num_classes):
    """
    Construct ResNeXt-152 for 3D images.
    """

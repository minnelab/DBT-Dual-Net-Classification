...
class CombinedBinaryModel(nn.Module):
    def __init__(self, alpha=0.5):
        ...
        # The alpha value determines the weight for the averaging of the two models' outputs
        self.alpha = alpha

    def forward(self, x):
        # Get the output from ResNet18 and AlexNet
        resnet_output = self.resnet18(x)  # Shape: (batch_size, 1)
        alexnet_output = self.alexnet(x)  # Shape: (batch_size, 1)
        
        # Compute the weighted average of the two model outputs
        combined_output = self.alpha * resnet_output + (1 - self.alpha) * alexnet_output
        
        # Apply sigmoid for binary classification
        final_output = self.sigmoid(combined_output)
        
        return final_output
    

model = CombinedBinaryModel()

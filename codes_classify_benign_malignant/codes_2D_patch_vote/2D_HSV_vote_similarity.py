...
def train_model(train_dataloader, val_dataloader, model, criterion, optimizer, num_epochs=10):
    ...
    log_file = "training_log_alexnet.txt"  # 日志文件路径
    ...
    for epoch in range(num_epochs):
        ...
        for rgb_images, labels in train_dataloader:
            ...
            for slices in rgb_images: 
                ...
                slice_preds = []
                for slice in slices:
                    outputs,similarity_loss = model(slice.unsqueeze(0))
                    predictions = outputs.squeeze()
                    bce_loss = F.binary_cross_entropy(predictions, labels)
                    loss_single = bce_loss *(1-model.similarity_weight )+ similarity_loss * model.similarity_weight

                    slice_preds.append(outputs.squeeze())
